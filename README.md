...
| AMATEUR ESPORTS UNION OCR Stat Tracking Feature Spec Frontend (Mini App + Web) + CRM Backend Architecture Version 1.0 \| February 2026 \| Addendum to Master Spec v3.0 + CRM Architecture v3.0 |
| --- |
...
| Expansion Rule New Tier 1 schemas are added based on game volume in AEU tournaments. When a Tier 2 game exceeds 500 cumulative matches, a structured schema is scoped and added in the next sprint cycle. |
| --- |
...
| type StatField = { key: string; // e.g. 'kills', 'adr', 'placement' label: string; // Display label in review UI type: 'integer' \| 'decimal' \| 'percentage' \| 'string'; required: boolean; // Required fields block submission if missing min?: number; // Validation bounds max?: number; }; type StatSchema = { schema_id: string; // e.g. 'schema_valorant' game_id: string; // Matches games_primary[] in player profile version: string; // Schema version — for future migrations fields: StatField[]; // Ordered array of stat field definitions }; type ExtractedStat = { key: string; raw_value: string; // Raw OCR output before normalization normalized_value: string \| number; // After type coercion confidence: number; // 0.0–1.0 AI confidence score flagged: boolean; // true if confidence < CONFIDENCE_THRESHOLD player_corrected: boolean; // true if player edited this field }; |
| --- |
...
- Default submission window: 2 hours after match result is confirmed in AEU system
...
| CRM Field submission_window_hours: Integer — configurable per platform (default 24). Stored in platform config, not per-user. |
| --- |
...
| type StatSubmission = { submission_id: UUID; // Unique submission record ID player_id: UUID; // Links to aeu_contact_id match_id: UUID; // Verified AEU match — confirmed result required tournament_id: UUID; // Parent tournament of the match game_id: string; // Detected or player-corrected game schema_id: string; // Schema used for extraction image_url: string; // Object storage URL of original screenshot extracted_stats: ExtractedStat[]; // Raw pipeline output — per-field player_confirmed_stats: Record<string, string\|number>; // Final values after player review submission_confidence: number; // Mean confidence across required fields (0.0–1.0) fraud_flagged: boolean; // Internal flag — never surfaced to player fraud_flag_reason?: string; // Admin-visible reason string review_status: 'pending' \| 'approved' \| 'rejected' \| 'auto_approved'; reviewed_by?: UUID; // Admin user_id if manually reviewed reviewed_at?: Timestamp; rejection_reason?: string; // Shown to player on rejection submitted_at: Timestamp; // Player confirmation time approved_at?: Timestamp; // When added to career_stats_by_game aggregate }; |
| --- |
...
| type StatReviewFormProps = { submission_id: string; game_id: string; // Used to label game + fetch schema game_detected: boolean; // True if auto-detected, false if player selected schema: StatSchema; // Field definitions for render order + validation extracted: ExtractedStat[]; // Pre-fills each field; confidence drives styling onConfirm: (confirmed: Record<string, string\|number>) => void; onRetake: () => void; // Triggers new upload — discards current extraction }; |
| --- |
...
| Revenue Logic The public stats panel is a meaningful Gold retention driver. Players who build stat history have a compounding reason to maintain Gold membership — their profile becomes more valuable over time. Loss of Gold = stats go private = strong retention pressure. |
| --- |
...
| AEU OCR Stat Tracking Feature Spec v1.0 \| February 2026 \| Addendum to Master Spec v3.0 + CRM Architecture v3.0 Confidential — Internal Use Only — Not for Distribution |
| --- |

| AMATEUR ESPORTS UNION OCR Stat Tracking — Sports Games Addendum Madden NFL \| EA Sports CFB \| EA FC \| NBA 2K \| NHL \| MLB The Show Team Stats Only — Frontend (Mini App + Web) + CRM Backend Schema Extension Version 2.0 \| February 2026 \| Addendum to OCR Stat Tracking Spec v1.0 |
| --- |
...
| // Sports schema extension — team stats only type SportsStatSchema = StatSchema & { mode_id: string; // e.g. 'h2h' \| 'mut_ranked' \| 'fut_rivals' \| 'dd_ranked' mode_label: string; // Display label: 'Head-to-Head' \| 'Ultimate Team Ranked' position_aware: false; // Always false — no position detection for sports games team_context: true; // Always true — all fields are user team aggregates }; // Extended submission record for sports games type SportStatSubmission = StatSubmission & { mode_id: string; // Detected or player-confirmed mode mode_detected: boolean; // true = auto-detected; false = player selected match_result: 'W' \| 'L' \| 'OTL' \| 'SOL' \| 'D'; // Game outcome from screenshot score_user: string; // User team final score score_opponent: string; // Opponent team final score flag_reason_mode?: string; // 'mode_undetected' \| 'madden_cfb_ambiguous' }; |
| --- |

| 2.1 Madden NFL schema_madden_h2h \| schema_madden_mut — Team Stats — Post-Game Team Summary Screen |
| --- |
...
| 2.2 EA Sports College Football (CFB) schema_cfb_h2h \| schema_cfb_cut — Team Stats — Post-Game Team Summary Screen |
| --- |
...
| Madden / CFB Disambiguation When game detection confidence < 0.80 between these two titles, apply a dedicated disambiguation step using college team logos, uniform design, field markings, and the EA CFB watermark. If unresolved, set flag_reason_mode = 'madden_cfb_ambiguous' and route to admin review. Never auto-assign to either title. |
| --- |
...
| 2.3 EA FC (FIFA) schema_eafc_h2h \| schema_eafc_fut — Team Stats — Post-Game Team Summary Screen |
| --- |
...
| 2.4 NBA 2K schema_2k_h2h \| schema_2k_myteam \| schema_2k_proam \| schema_2k_playnow — Team Stats |
| --- |
...
| 2.5 NHL schema_nhl_h2h \| schema_nhl_hut — Team Stats — Post-Game Team Summary Screen |
| --- |
...
| 2.6 MLB The Show schema_mlb_h2h \| schema_mlb_dd — Team Stats — Post-Game Box Score Screen |
| --- |
...
| No Position Selector | Not rendered for any sports schema. position_aware = false gates this component from rendering for all sports titles.                                                                                                             |
...
| AEU OCR Stat Tracking — Sports Games Addendum v2.0 \| February 2026 \| Addendum to OCR Stat Tracking Spec v1.0 Confidential — Internal Use Only — Not for Distribution |
| --- |
...
| USER TYPE 01 Player Workflow Free + Player Premium \| 13 Steps |
| --- |
...
| USER TYPE 02 Organization Premium Workflow Org Admin + Teams \| 13 Steps |
| --- |
...
| USER TYPE 03 Tournament Organizer Workflow TO Membership \| 10 Steps |
| --- |
...
| USER TYPE 04 Sponsor Workflow Sponsor Account \| 5 Steps |
| --- |
...
| --- | --- | --- |
| Access currency Incentive currency Reward currency Discount currency Governance signal Liquidity retention tool Sponsor-funded engagement engine | An investment A profit expectation A speculative asset | Reduces cash withdrawals Increases in-platform reuse Increases session frequency Increases conversion velocity Enables sponsor-funded engagement Drives gamified loyalty Builds measurable internal economy |

| The Most Powerful Part When done correctly, tokens turn emotional peaks — wins, losses, clutch moments — into purchasing behavior without forcing cash friction. That is the real utility. The token is the bridge between competitive feeling and commercial action. |
| --- |
...
| Design Principle Each user type needs its own distinct gamification layer — different progression bars, different badge logic, different token incentives, different dashboard experience. We layer on top of this: social visibility, token economics, marketplace loops, Twitch-style live reinforcement, and cross-user system intersections. |
| --- |
...
| **Match Highlight Auto-Share** | Clip sharing with one tap to Twitter, Discord, TikTok.                 | Organic platform marketing — competitive moments drive external discovery |

Community Clip Feed TBD - Discuss with Jonathan

Need to figure out a clipping system
...
| **Category Exclusivity Lock** | First sponsor in a category to reach Gold tier locks out same-tier competitors.        | Scarcity mechanic — creates urgency for brands to move fast before competitor locks them out |
...
Can we create some H2H competitions between sponsors?
...
| AEU Unified Identity Competitive progression engine · Social status network · Token-powered incentive economy · Sponsor-funded engagement machine · Marketplace-integrated ecosystem · Operational reputation platform Each user type sees a different dashboard. Underneath: one unified behavioral architecture. |
| --- |
...
| Why this order matters Most Web3 projects launch the blockchain token first and try to build utility around it after. That's backwards. By launching the native token first, we tune the entire economy with real behavioral data before opening the Web3 layer. We know our earn rates, redemption patterns, and what players actually engage with — instead of guessing. |
| --- |

| LAYER 1 The Native Token Platform currency. Controlled environment. Zero blockchain friction. |
| --- |
...
| Players Earn By • Winning matches and tournaments • Completing performance challenges • Daily check-ins and streaks • Referring new players • Purchasing sponsor products • Keeping winnings in wallet | Players Spend On • Hybrid tournament entry (USD + tokens) • Premium tournament access • Sponsor gear discounts • Token-exclusive marketplace drops • Ad-free premium stream viewing • Micro-tips to players during streams |
| --- | --- |
...
| LAYER 2 The Web3 Token Ownership. Transferability. Staking. Scarcity. |
| --- |
...
| Web3 Utilities • Staking for sponsor placement boosts • Token-gated premium tournament access • Marketplace limited drops (true scarcity) • Org stake signal for sponsor matchmaking • Governance signals (platform voting) • Cross-platform transferability | Stays Native Only • Daily login rewards • Referral bonuses • Partial refunds in tokens • Profile completion rewards • Feedback submission rewards • Re-entry bonuses |
| --- | --- |

## The Legal Line

| Critical positioning — must be maintained consistently The Web3 token must always be positioned as: Utility \| Reward \| Access \| Incentive It must never be positioned as: An investment \| A profit-generating asset \| A speculative instrument This distinction is not just marketing — it is the difference between a utility token and a security under SEC guidelines. Every document, pitch, and communication about the Web3 token needs to maintain this framing. |
| --- |
...
**<span class="smallcaps">AMATEUR ESPORTS UNION</span>**
...
| SCOPE DECISION This affiliate program is membership conversion only. Payouts trigger exclusively when a referred user converts to a paid Premium membership. All other affiliate triggers from the v1 architecture (rake share, GMV, marketplace, tokens) are deferred. This decision simplifies the model, eliminates the commission stack problem, and makes financial modeling tractable before launch. |
| --- |
...
| Subscription Price (Monthly) | One-Time Payout (TBD %) | Monthly Recurring (TBD %) | Months Active | Max Recurring Earned | Total Max Per Referral |
| --- | --- | --- | --- | --- | --- |
...
**1. PLAYER MEMBERSHIP FEATURES**

**Monthly - \$9.99 6 month - \$49.99**
...
| Free tournament entries (monthly cap) (Automated tournaments) (tokens) | ❌ | ✅ | Subscription incentive |
| Access to member-only tournaments | ❌ | ✅ Stakes x1 | Scarcity + upgrade pressure |
| League & seasonal competition eligibility (AEU automated leagues) | ❌ | ✅ | Retention driver |
| Ranked matchmaking access | ❌ | ✅ | Competitive integrity |
| Featured match eligibility (non-stream) | ❌ | ✅ | Placement / visibility |
| Player stats & performance, ranking history (dashboard) | ❌ | ✅ (full + export) | Data value |
| Leaderboards (view only) | ✅ | ✅ | Public engagement |
| Make a Team | Join Only | ✅ | One team per game |
| Leaderboards (ranked placement eligibility) (scouting) | ❌ | ✅ | Prestige gating |

**2. TOURNAMENT ORGANIZER (TO) FEATURES**

**Gold; Monthly - \$29.99 , 6 Month - \$149.99 Yearly - \$299.99**

**Platinum: Monthly - \$97.99 6 Month - \$449.99 Yearly - \$799.99**
...
| Staff / moderator assignment                                         | ✅ | ✅ | ✅                            | Ops-heavy                      |
...
| Where sponsorship can be placed | Anywhere available in marketplace | Anywhere available in marketplace + Custom placements via inquiry | Controlled scale |
...
## **Organization Membership Features Matrix (Free vs Gold vs Platinum)(Can buy TO memberships)**
...
# Summary — What Is All of This?

## What AEU Is

The **Amateur Esports Union (AEU)** is a platform being built by Most Vicious Players LLC that lets amateur and casual gamers compete in organized tournaments, leagues, and ranked play — with real money entry fees, real prize payouts, and a membership structure similar to how a gym or sports league works. It runs on both a **website** and a **Telegram Mini App**, sharing one unified design system.

## Who Uses It

There are four main user types, and each has its own dashboard, features, and membership tiers:

- **Players** — Sign up free or pay for Gold membership ($9.99/mo) to unlock ranked play, leagues, stat tracking, team creation, and tournament perks.
- **Tournament Organizers (TOs)** — Host and manage brackets, charge entry fees, run leagues, and handle prize payouts. Free tier is limited to 1 tournament; Gold ($29.99/mo) and Platinum ($97.99/mo) unlock more capacity, branding tools, and sponsor slots.
- **Organizations (Orgs)** — Esports teams/clubs that manage rosters, budgets, and multiple teams. Gold and Platinum tiers scale the number of teams, admin seats, and analytics access.
- **Sponsors** — Brands that pay to place logos, links, and campaigns on tournaments, org pages, and across the platform. Base tier is free (with approval); Platinum is custom/enterprise.

## How AEU Makes Money

Revenue comes from multiple streams layered together: paid memberships (monthly/yearly subscriptions), tournament entry fee rake, a marketplace for digital goods and services, token purchases (used for entries, boosts, and cosmetics), sponsor placements sold as self-serve or enterprise deals, and affiliate commissions on membership referrals.

## What the Documents Cover

- **Part 1 (Master Spec)** — The full UI/UX blueprint. Every screen, component, dashboard layout, button, notification, and workflow for the web app and Telegram app. Includes a task-by-task execution tracker showing what's built and what's not.
- **Part 2 (CRM Architecture)** — The backend brain. How user data, roles, memberships, wallets, tournaments, payouts, and compliance are structured in the database. Defines every API endpoint, webhook, and automation rule.
- **Part 3 (OCR Stat Tracking)** — A feature where players photograph their end-of-match scoreboard and the system uses OCR (image-to-text AI) to automatically extract and verify stats. Covers supported games (Madden, NBA 2K, FC 25, Call of Duty, Fortnite, Apex Legends), fraud detection, and how stats flow into player profiles.
- **Part 4 (Member Workflows)** — The step-by-step lifecycle of every user type: how they sign up, pay, compete, earn, and stay engaged. Also covers the marketplace, token economy, gamification (XP, badges, streaks, leaderboards), and how all the systems connect into a single engagement loop.
- **Part 5 (Affiliate Program)** — The referral system. When someone you refer converts to a paid membership, you earn a one-time payout plus a small recurring monthly cut (capped at 12 months). Same structure for all user types. Exact percentages are still TBD.
- **Part 6 (Feature Access Matrix)** — A quick-reference chart showing exactly which features are available at each membership tier (Free, Gold, Platinum) for Players, TOs, Sponsors, and Orgs.

## The Bottom Line

AEU is building a full-stack competitive gaming platform — think a combination of a tournament bracket site, a sports league management tool, a marketplace, and a membership rewards program, all wired together with real money flowing through it. These documents are the complete technical and business blueprint for how every piece of that system works, looks, and gets built.
